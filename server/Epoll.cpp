...
bool Epoll::addFd(int fd,uint32_t events){
    ...
    ev.events = events;
    // 添加文件描述符fd，并监听其ev事件
    int result=epoll_ctl(Epoll::instance()->fd, EPOLL_CTL_ADD, fd, &ev);
    // 调用成功返回0
    return (result==0);
}
...
bool Epoll::delFd(int fd,uint32_t events){
    epoll_event ev = {0};
    int result=epoll_ctl(Epoll::instance()->fd, EPOLL_CTL_DEL, fd, &ev);
    return (result==0);
}
...
bool Epoll::modFd(int fd,uint32_t events){
    ...
    ev.events = events;
    // 将文件描述法fd的监听事件改为ev
    int result=epoll_ctl(Epoll::instance()->fd, EPOLL_CTL_MOD, fd, &ev);
    return (result==0);
}

int Epoll::wait(int timeoutMS){
    // 监听就绪的文件描述符，并返回就绪的文件描述符数量
    // timeoutMS是超时时间，传入-1表示不会超时（即如果没有就绪的文件描述符，就一直阻塞）
    // 用传入参数events保存这次就绪的文件描述法
    return epoll_wait(Epoll::instance()->fd, &events[0], static_cast<int>(events.size()), timeoutMS);
}
