...
def queryObj(conn,table,obj):
	# 根据表名和对象（字典）构造select语句并执行
	strlist=['select * from ',table]
	if(len(obj)>0):
...
def queryJson(conn,table,str):
	# 根据表名和json字符串构造select语句并执行
	obj=json2obj(str);
	return queryObj(conn,table,obj)
...
#以下为数据库更新函数（增删改），更新成功返回True，更新失败返回False
